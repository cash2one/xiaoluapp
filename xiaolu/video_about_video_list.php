 ...
 * @file: video_about_video_list.php
 * @author: Chen Zhong
 * @charset: UTF-8
...
$mydata['tag_arr'] = get_param('tag_arr',false); //标签json串
$mydata['tag_arr'] = json_decode(stripslashes($mydata['tag_arr']),true);
...
if(!empty($mydata['anchorid'])){
    $where .= " AND `vvl_uid` = ".$mydata['anchorid'];
}
...
//排序
$orderby =  ' ORDER BY `vvl_last_week_plays` DESC';

//查找用户加标签相关视频数
$data_count_key = md5("xl_about_video_ut_count_".$where);
$data_count = $mem_obj->get($data_count_key);
if($data_count === false){
    $sql = "SELECT COUNT(DISTINCT `id`) AS num FROM `video_video_list` ".$where;
...
//获取视频更多数据
$data_key = md5("xl_about_video_list_data_".$where.$orderby.$limit);
$data = $mem_obj->get($data_key);
if($data === false){
    $sql = "SELECT `id`,`vvl_title`,`vvl_imgurl`,`vvl_imgurl_get`,`vvl_category_id`,`vvl_game_id`,`vvl_type_id`,`vvl_upload_time`,`in_date`,
            `vvl_tags`,`vvl_author_id`,`vvl_count`,`vvl_video_id`,`vvl_uid`,`vvl_tags`,`vvl_time`,`vvl_up_num`,`vvl_down_num`,`vvl_sourcetype`
            FROM `video_video_list` AS B ".$where.$orderby.$limit;
    $data = $conn->find($sql);
