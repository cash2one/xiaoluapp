...
//查询条件
$where = " WHERE vvl_uid = ".$mydata['uid'];
if(!empty($game_id)){
    $where .= ' AND `vvl_sourcetype` = 14 AND `vvl_game_id` = '.$game_id;
}

//查询数据总数
$my_video_list_count_key = 'sdk_my_video_list_count_'.$mydata['uid'].'_'.$game_id; //我的视频总数缓存key  'my_video_list_count_' + $mydata['uid']
...
//获取我的视频数据
$my_video_list_data_key = 'sdk_my_video_list_data_'.$mydata['uid'].'_'.$mydata['pagenum'].'_'.$mydata['pagesize'].'_'.$game_id; //我的视频数据缓存key  'my_video_list_data_' + 用户id + 当前页 + 每页显示数据
...
if($data === false){
    $sql = "SELECT A.id,A.vvl_title,A.vvl_imgurl,A.vvl_imgurl_get,A.vvl_category_id,A.vvl_type_id,A.vvl_author_id,A.in_date,
            A.vvl_tags,A.vvl_last_week_plays,A.vvl_last_month_plays,vvl_server_url,vvl_md5file,A.vvl_count,A.vvl_game_id,B.`gi_name`,A.`vvl_uid`
...
if(!empty($data)){
    foreach($data as $val){
        ...
        $json = array(
            ...
            'tag' => $tag_name, //视频标签
            'filemd5' => $val['vvl_md5file'],
            'aliurl' => $val['vvl_server_url'],
