...
function uc_user_register($username, $password, $mac='',$source=1,$email='', $questionid = '', $answer = '', $regip = '') {
	return call_user_func(UC_API_FUNC, 'user', 'register', array('username'=>$username, 'password'=>$password,'mac'=>$mac, 'source'=>$source, 'email'=>$email, 'questionid'=>$questionid, 'answer'=>$answer, 'regip' => $regip));
}

function uc_reset_pwd($username,$password){
    return call_user_func(UC_API_FUNC, 'user', 'reset_pwd', array('username'=>$username, 'password'=>$password));
}

function uc_get_phone_code($phone,$code){
    $return = call_user_func(UC_API_FUNC, 'phone', 'get_code', array('phone'=>$phone,'code'=>$code));
    return UC_CONNECT == 'mysql' ? $return : uc_unserialize($return);
}
 ...
 * @return:
 * @author: Chen Zhong
 * @create: 2015-11-12 11:55:12
 **/
function uc_user_ls($page = 1, $pagesize = 20,$gameid) {
    $return = call_user_func(UC_API_FUNC, 'user', 'ls', array('page'=>$page, 'pagesize'=>$pagesize,'gameid'=>$gameid));
    return UC_CONNECT == 'mysql' ? $return : uc_unserialize($return);
}
 ...
 * @create: 2015-11-12 11:55:12
 **/
function uc_user_count($gameid) {
    $return = call_user_func(UC_API_FUNC, 'user', 'usercount',array('gameid'=>$gameid));
    return UC_CONNECT == 'mysql' ? $return : uc_unserialize($return);
}

/**
 * @name:uc_get_reg_phone_code
 * @description: 注册时获取验证码
 * @param: $phone=手机号码
 * @return: 
 * @author: Xiong Jianbang
 * @create: 2015-9-18 下午4:02:12
 **/
function uc_get_reg_phone_code($phone) {
    $return = call_user_func(UC_API_FUNC, 'phone', 'get_reg', array('phone'=>$phone));
    return UC_CONNECT == 'mysql' ? $return : uc_unserialize($return);
}


function get_reset_pwd_code_by_phone($phone) {
    $return = call_user_func(UC_API_FUNC, 'phone', 'get_reset_pwd', array('phone'=>$phone));
    return UC_CONNECT == 'mysql' ? $return : uc_unserialize($return);
}


function exist_phone_user_info($phone) {
    $return = call_user_func(UC_API_FUNC, 'phone', 'exist_info', array('phone'=>$phone));
    return UC_CONNECT == 'mysql' ? $return : uc_unserialize($return);
}
...
/**
 * @name:uc_insert_phone_code
 * @description: 插入手机数据
 * @param: $phone=手机号码
 * @param:$type=类型0：注册 1：忘记密码
 * @return: 
 * @author: Xiong Jianbang
 * @create: 2015-9-18 下午2:18:38
 **/
function uc_insert_phone_code($phone,$code='', $type=0,$model='',$brand='',$gpu='',$systemversion='',$cpu='',$deviceid='',$packagename='',$ip='') {
    $isuid = intval($isuid);
    $return = call_user_func(UC_API_FUNC, 'phone', 'insert', array(
            'phone'=>$phone, 'type'=>$type,
            'code'=>$code,'model'=>$model,
            'brand'=>$brand,'gpu'=>$gpu,
            'systemversion'=>$systemversion,'cpu'=>$cpu,
...
function uc_update_phone_code($id,$arr=array()) {
    $id = intval($id);
    $return = call_user_func(UC_API_FUNC, 'phone', 'update', array(
            'id'=>$id, 'arr'=>$arr
    ));
    return UC_CONNECT == 'mysql' ? $return : uc_unserialize($return);
}
...
/**
 * @name:uc_user_login
 * @description: 用户名或者手机号登录
 * @return: 
 * @author: Xiong Jianbang
 * @create: 2015-9-17 上午11:37:17
 **/
function uc_user_login($username, $password, $isuid = 0, $checkques = 0, $questionid = '', $answer = '',$loginip='') {
	$isuid = intval($isuid);
	$return = call_user_func(UC_API_FUNC, 'user', 'login', array('username'=>$username, 'password'=>$password,
	         'isuid'=>$isuid, 'checkques'=>$checkques, 
	        'questionid'=>$questionid, 'answer'=>$answer,'loginip'=>$loginip));
	return UC_CONNECT == 'mysql' ? $return : uc_unserialize($return);
}
...
function uc_user_info_edit($username, $nickname='',$gender='',$desc='') {
    return call_user_func(UC_API_FUNC, 'user', 'info_edit', array('username'=>$username, 'nickname'=>$nickname, 'gender'=>$gender, 'desc'=>$desc));
}
...
function uc_get_user_info($username, $isuid=0) {
    $return = call_user_func(UC_API_FUNC, 'user', 'get_user_info', array('username'=>$username, 'isuid'=>$isuid));
    return UC_CONNECT == 'mysql' ? $return : uc_unserialize($return);
}

function uc_get_same_nickname($nickname,$phonenumber) {
    $return = call_user_func(UC_API_FUNC, 'user', 'get_same_nickname', array('nickname'=>$nickname,'phonenumber'=>$phonenumber));
    return UC_CONNECT == 'mysql' ? $return : uc_unserialize($return);
}
