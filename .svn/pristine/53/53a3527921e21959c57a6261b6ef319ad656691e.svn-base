...
if($game_count > 0){
    ...
    $save_game_info = array();
    for ($i = 0;$i < $game_count;$i++){
        //如果是过滤包名，退出
        if($game_list[$i]['packagename']=='com.ratrodstudio.snowparty'){
            continue;
        }
        ...
        //查询大于更新游戏版本号的游戏信息
        $PackageName = $game_info[$i]['packagename']; //游戏包名
        $VersionCode = $game_info[$i]['versioncode']; //游戏版本号
        $sql = "SELECT * FROM `mzw_game_version` WHERE (FIND_IN_SET(2,gv_client_type)>0 OR FIND_IN_SET(3,gv_client_type)>0)
                AND gv_package_name='$PackageName' AND ((gv_version_no>$VersionCode AND gv_m_status=1) OR gv_version_no=$VersionCode)
                ORDER BY gv_version_no DESC";
        ...
        if(count($_row) > 1){
            ...
            $tmp_sql_gpu_in = 'SELECT * FROM mzw_game_downlist
                               WHERE mgd_client_type!=1 AND gv_id IN ('.$tmp_gv_id_list.') AND mgd_package_type!=2 AND '.$tmp_find_gpu_id
                                .' ORDER BY mgd_package_type DESC,mgd_package_up_v_code DESC';
            ...
            $tmp_new_game = ''; //找出最新的适配的可升级版本
            $tmp_old_gv_id = ''; //旧版本游戏的gv_id
            $tmp_old_game = ''; //旧版本的适配版本
            $tmp_tmp_old_game = ''; //临时的旧版本downlist数据
            ...
            if($tmp_new_game['mgd_apk_agsin']==$tmp_old_game['mgd_apk_agsin']){
                //查游戏是否有增量包
                $sql_diff = 'SELECT * FROM `mzw_game_patch_diff` WHERE client_type != 1 AND from_gv_id='.$tmp_old_game['gv_id'].' AND from_mgd_id='.$tmp_old_game['mgd_id']
            ...
            if($tmp_new_game['mgd_package_type']==1){
                ...
                $sql_obb_old = 'SELECT * FROM `mzw_game_patch`
        				        WHERE client_type != 1 AND gv_id='.$tmp_old_game['gv_id'].' AND mgd_id='.$tmp_old_game['mgd_id'];
                $_row_obb_old = $conn->find($sql_obb_old);

                //查新版本的OBB之类的文件
                $sql_obb_new = 'SELECT * FROM `mzw_game_patch`
        				        WHERE client_type != 1 AND gv_id='.$tmp_new_game['gv_id'].' AND mgd_id='.$tmp_new_game['mgd_id'];
                ...
                if(count($_row_obb_old)>0 && count($_row_obb_new)>0 && !is_empty($tmp_diff_info)){
                    foreach ($_row_obb_new as $v_obb_new){
                        ...
                        foreach ($_row_obb_old as $v_obb_old){
                            //如果MD5值相等，则不需要更新了
                            if( $v_obb_new['patch_md5']==$v_obb_old['patch_md5'] && $v_obb_new['file_type']==$v_obb_old['file_type']){
            ...
            */
            ...
            $tmp_sql = 'SELECT A.img_key,A.path as src_path,B.img_path as path,A.type FROM mzw_game_screenshot A
                            ...
                            WHERE A.gv_id = '.$tmp_new_game['gv_id'].' AND A.type = 7 AND B.size_id = 22 ORDER BY B.id DESC';
            ...
            $json = array(
                ...
                'updateContent' => $_row[$tmp_new_game['gv_id']]["gv_update_text"]//游戏更新信息
            );
            $returnArr['rows'][]=$json;
            $update_num++;
            //记录更新日志用的
