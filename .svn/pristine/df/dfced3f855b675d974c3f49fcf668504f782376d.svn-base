<?php
/*=============================================================================
#     FileName: tmp_action.sdk_sim_handle.count.in.php
#         Desc: 定期把记录在文件中的数据记录(/countapi/sdk_login_count.php)数据存在数据库存中（SDK模拟手柄统计）
#       Author: Chen Zhong
#        Email: chenzhong@kuaiyouxi.com
#   LastChange: 2015-07-11 11:56:48
#      History:
=============================================================================*/
...
//组合要入库文件的路径
$path = WEBPATH_DIR."data/sdk_sim_handle/data".$yes_date."_".$tmp_aa.".dat";

//随机取内容
$tmp_arr = file($path);
if(is_array($tmp_arr)){
    $tmp_table_name = "kyx_sdk_sim_handle_log";//数据表名字
    $str_sql = "insert into ".$tmp_table_name."(`sshl_in_date`, `sshl_md`, `sshl_bd`, `sshl_dc`, `sshl_sdkv`,
			`sshl_sdkbv`, `sshl_vc`,`sshl_vn`,`sshl_pn`,`sshl_title`,`sshl_mac`,`sshl_chl`,`sshl_eid`,`sshl_ct`,
			`sshl_ut`,`sshl_st`,`sshl_ip`,`sshl_imei`,`sshl_nip`,`sshl_port`,`sshl_wifiname`,`sshl_time`,`sshl_nte`)values";
    ...
    foreach($tmp_arr as $v){
        if(!empty($v)){
            ...
            $tmp_sql_val .= ",'".str_replace('\"','',$val['wifiname'])."'";//获取客户端的IP
            $tmp_sql_val .= ",".intval($val['time'])."";//获取客户端的IP
    ...
    unset($tmp_arr);
    echo('导入sdk_sim_handle数据成功');
}else{
    echo('没有可导入的sdk_sim_handle数据');
}
