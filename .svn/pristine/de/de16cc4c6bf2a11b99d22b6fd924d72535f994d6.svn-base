<?php
/*=============================================================================
#     FileName: update_video_vert_common_redis_date.php
#         Desc: 视频广告定期统计信息
#       Author: Chen Zhong
#        Email: chenzhong@kuaiyouxi.com
#   LastChange: 2015-09-17 15:37:48
#      History:
=============================================================================*/
...
include_once(WEBPATH_DIR."video_vert.event.config.inc.php"); //统计参数配置
...
if(!empty($tmp_id) && isset($video_vert_redis_param[$tmp_id]) && !empty($video_vert_redis_param[$tmp_id])){
    $temp_sdk_param = $video_vert_redis_param[$tmp_id];
    $video_vert_redis_param = array();
    $video_vert_redis_param[] = $temp_sdk_param;
}

if(!empty($video_vert_redis_param)){
    foreach($video_vert_redis_param as $val){
        ...
        //拼接其他sql
        $sql .= "`".$val['field_pre']."_title`,`".$val['field_pre']."_pn`,`".$val['field_pre']."_vc`,`".$val['field_pre']."_chl`,`".$val['field_pre']."_adtitle`,`"
                .$val['field_pre']."_md`,`".$val['field_pre']."_adid`,`".$val['field_pre']."_source` "."
        ...
        if(!empty($data)){
            ...
            foreach ($data as &$tval){
                ...
                //检查是否有记录这个盒子型号
                $redis_key = md5('kyxmodel|'.$tval[$val['field_pre'].'_md']);
                $redis_ok = $redis->get($redis_key);
                //如果没有找到，则插入
                if(!$redis_ok){
                    //把数据插入盒子型号信息表
                    $arr = array(
                        'm_in_date'=>$this_date,//'记录日期',
                        'm_name'=>$tval[$val['field_pre'].'_md'],//'盒子型号名称',
                        'm_name_cn'=>$tval[$val['field_pre'].'_md'],//'盒子型号名称',
                        'm_order'=>0//'排序号',
                    );
