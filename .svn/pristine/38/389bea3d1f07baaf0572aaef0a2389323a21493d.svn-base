...
$data = array();
//查游戏对应版本的信息
$sql = "SELECT gv_id as appid,g_id as gid,gv_type_id as tid,gv_title as title,gv_title_en as title_en,gv_version_no as versioncode,gv_version_name as version,
	gv_update_time as updatetime,gv_package_name as  packagename,gv_ico_key as icon,gv_down_nums as downloadscount,
	gv_description as description,gv_description_en as description_en,gv_app_prop as prop,gv_notice as attention,gv_update_text as updateContent, 
        gv_lang as language  FROM `mzw_game_version`  WHERE gv_status = 1 AND (FIND_IN_SET(1,gv_client_type)>0 OR FIND_IN_SET(3,gv_client_type)>0) AND gv_id=".$mydata['appid'];

//	case gv_lang when 0 then 'English' when 1 then '中文' when 2 then '日本语' when 3 then '韩文' when 4 then 'other' end language
...
//if( $data_arr){
	...
	//查所属分类的名称
	$tmp_sql = 'SELECT t_name_cn as name,t_name_en as name_en FROM mzw_game_type WHERE t_id='.$data["tid"];
	...
	$json = array(
			...
			'corver'=>isset($tmp_hot_arr[$data["appid"]][2][0]) ? $tmp_hot_arr[$data["appid"]][2][0] : '',//专题大图
			'icontvpath' =>isset($tmp_hot_arr[$data["appid"]][3][0]) ? $tmp_hot_arr[$data["appid"]][3][0] : '',//TV游戏大图
			'icontvindex' =>isset($tmp_hot_arr[$data["appid"]][5][0]) ? $tmp_hot_arr[$data["appid"]][5][0] : '',//首页大图
			'icontvdetail' =>isset($tmp_hot_arr[$data["appid"]][6][0]) ? $tmp_hot_arr[$data["appid"]][6][0] : '',//详情页大图
...
// 			'language'=> $language?$language:'English',//游戏语言
	...
	switch ($mydata['language']) {
		case 'zh':
		    $json['title'] = $data["title"]; //游戏标题
		    $json['description'] = $data["description"];//游戏描述
		    if($language==0){
		        $json['language'] = '英文';
		    }elseif($language==1){
		        $json['language'] = '中文';
		 ...
		 break;
		 case 'en':
		    $json['title'] = $data["title_en"]; //英文游戏标题
		    $json['description'] = $data["description_en"];//英文游戏描述
		    if($language==0){
