...
define('UC_USER_PHONE_FORMAT_ILLEGAL', -7);//手机格式错误
...
class usercontrol extends base {
	...
	/**
	 * @name:onregister
	 * @description: 安卓客户端的注册 接口，不需要验证邮箱
	 * @param: 
	 * @return: 
	 * @author: Xiong Jianbang
	 * @create: 2015-9-19 下午3:42:45
	 **/
	function onregister() {
		$this->init_input();
		...
		$mac = $this->input('mac');
		if(($status = $this->_check_username($username,$source)) < 0) {
			return $status;
		}
		$uid = $_ENV['user']->add_user($username, $password, $email, 0, $questionid, $answer, $regip,$mac,$source);
		return $uid;
	}
	...
	function oninfo_edit() {
	    ...
	    $status = $_ENV['user']->edit_user_info($username, $nickname, $gender, $desc);
	    return $status;
	}
	...
	function onlogin() {
		...
		if($isuid == 1) {
			...
			$user = $_ENV['user']->get_user_by_username($username); //用户名或者手机
		}
		$passwordmd5 = preg_match('/^\w{32}$/', $password) ? $password : md5($password);
		if(empty($user)) {
			$status = -1; //-1:用户不存在，或者被删除
			...
			$status = $user['uid'];
			//修改登录记录
			$_ENV['user']->edit_user_login_info($username, $loginip);
		}
		$merge = $status != -1 && !$isuid && $_ENV['user']->check_mergeuser($username) ? 1 : 0;
		return array($status, $user['username'],  $user['phone'], $merge);
// 		return array($status, $user['username'], $password, $user['email'], $merge);
	}

	function onreset_pwd(){
	    $this->init_input();
	    $username = $this->input('username');
	    $password = $this->input('password');
	    $user = $_ENV['user']->reset_pwd($username,$password);
	    return $user;
	}
	 ...
	 * @name:onget_user
	 * @description: 获取用户信息
	 * @return: 第一位：用户ID，第二位：用户名称，第三位：email，第四位:密码，第五位，salt盐
	 * @author: Xiong Jianbang
	 * @create: 2015-11-4 下午2:36:45
	 **/
	function onget_user() {
		$this->init_input();
		$username = $this->input('username');
	...
	function onget_user_info() {
	    ...
	    if($status) {
	        return array(
	                'uid'=>$status['uid'],'username'=>$status['username'],
	                'email'=>$status['email'],'nickname'=>$status['nickname'],
	                'gender'=>$status['gender'],'desc'=>$status['desc'],
	        );
	    } else {
	        return FALSE;
	    }
	...
	/**
	 * @name:onget_same_nickname
	 * @description: 返回是否有相同的昵称
	 * @author: Xiong Jianbang
	 * @create: 2015-11-6 上午10:42:42
	 **/
	function onget_same_nickname() {
	    $this->init_input();
	    $nickname = $this->input('nickname');
	    $phonenumber = $this->input('phonenumber');
	    $status = $_ENV['user']->get_same_nickname($nickname,$phonenumber);
	    return $status;
	}
	...
	function _check_username($username,$source=1) {
		$username = addslashes(trim(stripslashes($username)));
		if($source==2){//如果是手机注册，则只检查手机号
    		if(!$_ENV['user']->check_phone($username)) {
		...
		if(!$_ENV['user']->check_usernamecensor($username)) {
    ...
    function onls() {
        ...
        $gameid = intval($this->input('gameid'));
        $pagesize = $pagesize ? $pagesize : UC_PPP;
        $conditions = 'WHERE (source = 2 OR source = 3) AND is_recommed = 2 AND FIND_IN_SET('.$gameid.',video_game)>0 AND is_show = 1 ORDER BY sort DESC';
        $data = $_ENV['user']->get_user_list($page, $pagesize, $conditions);
        return $data;
    }

    function onusercount() {
        $gameid = intval($this->input('gameid'));
        $conditions = 'WHERE (source = 2 OR source = 3) AND is_recommed = 2 AND is_show = 1 AND FIND_IN_SET('.$gameid.',video_game)>0';
        $totalnum = $_ENV['user']->get_user_totalnum($conditions);
        return $totalnum;
    }
