...
$tmp_sql = "SELECT MAX(gd.`mgd_id`) AS `mgd_id` FROM `mzw_game_version` AS gv
			...
			WHERE gd.`mgd_client_type` != 2 AND gd.`mgd_package_type` != 2 AND (FIND_IN_SET(1,gv.gv_client_type)>0 OR FIND_IN_SET(3,gv.gv_client_type)>0)
			AND gv.`gv_pinyin_simple` LIKE '%".$mydata['searchkey']."%' GROUP BY gv.`gv_id`";
...
if(!empty($tmp_downlist_mgd)){
    ...
    $gpu_id_sql = "SELECT DISTINCT `mgd_id` FROM mzw_game_downlist
                   WHERE `mgd_client_type` != 2 AND `mgd_id` IN (".$tmp_downlist_mgd_str.") AND ".$tmp_find_gpu_id;
...
$sql = "SELECT count(1) as num FROM `mzw_game_version` gv
        ...
        WHERE (FIND_IN_SET(1,gv.gv_client_type)>0 OR FIND_IN_SET(3,gv.gv_client_type)>0) AND gv.gv_status=1 $where ";
...
$sql_data = "SELECT gv.gv_id as vid, gv.gv_type_id as tid,gv.gv_title as vtitle,gv.gv_version_no as versioncode,
			 ...
			 " FROM `mzw_game_version` gv LEFT JOIN `mzw_game_downlist` gd ON gv.gv_id = gd.gv_id WHERE (FIND_IN_SET(1,gv.gv_client_type)>0 OR FIND_IN_SET(3,gv.gv_client_type)>0) AND gv.gv_status=1 $where $orderby LIMIT $ParamPage,".$mydata['pagesize'];
