...
$mydata['order'] = intval(get_param('order')); //排序（0：更新时间（默认） 1：总下载数）
$mydata['tid'] = intval(get_param('tid')); //分类id（属性分类 -1：最新游戏 -2:大型游戏 -3：模拟器游戏 -5：多人游戏 -6：遥控器游戏）
$mydata['sid'] = intval(get_param('sid')); //二级分类
...
if($mydata['tid'] > 0){
    ...
    $tid_data = $conn->get_one($tid_sql);
    $tid_name = $tid_data['t_name_cn'];
}else{
    switch($mydata['tid']){
        case '-2': //大型游戏
        ...
        case '-3': //模拟器游戏
            if(!empty($mydata['sid'])){
                $temp_where .= " AND  FIND_IN_SET(".$mydata['sid'].",gv.gv_nes_property)>0 ";
            }else{
...
if($mydata['size']!=0 && $mydata['size']!=1){
    ...
    $tmp_sql_gpu_in = 'SELECT `gv_id` FROM `mzw_game_downlist` WHERE mgd_client_type != 1 AND mgd_package_type != 2 AND '.$tmp_find_gpu_id.$tmp_find_model_id.' GROUP BY gv_id HAVING max(mgd_game_size)'.$mydata['size'];
}else{
...
$returnArr = array(
    ...
    'pagecount' => 0, //总页数
    'id' => intval($mydata['tid']), //分类id
    'sid' => intval($mydata['sid']), //二级分类id
    'title' => $tid_name, //分类名称
    'sdatas' => array(), //二级分类数组
    'error'=>'',
    'rows' => array() //数据
