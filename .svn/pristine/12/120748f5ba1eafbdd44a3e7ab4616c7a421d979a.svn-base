...
/**
 * @copyright: @快游戏 2014
 * @description: 获取最新视频列表
 * @file: video_new_video_list.php
 * @author: Chen Zhong
 * @charset: UTF-8
...
$mydata['isflush'] = intval(get_param('isflush')); //是否刷新（默认0：不刷新 >0：刷新）
...
if(empty($mydata['gameid'])){
    ...
    //查询数据总数
    $video_new_video_list_count_key = 'mzw_video_new_video_list_count_'.$mydata['gameid']; //最新视频总数缓存key  'mzw_video_new_video_list_count_' + 游戏id
    $data_count = $mem_obj->get($video_new_video_list_count_key); //最新视频总数
    if($data_count === false){
        $sql = "SELECT count(1) as num FROM `video_video_list` AS A ".$where ;
        $data_count = $conn->count($sql);
        $mem_obj->set($video_new_video_list_count_key,$data_count,1800);
    }
}

//排序
if(empty($mydata['gameid']) && $mydata['isflush'] > 0){
    $five_date = date('Ymd',mktime(23,59,59,date('m'),date('d') - 5,date('Y'))); //5天前日期
    $where .= " AND A.`in_date` > ".$five_date;
...
//获取视频更多数据
$video_new_video_list_data_key = 'mzw_video_new_video_list_data_'.$mydata['gameid'].'_'.$mydata['pagenum'].'_'.$mydata['pagesize']; //最新视频数据缓存key  'mzw_video_new_video_list_data_' + 游戏id + 当前页 + 每页显示数据
$data = $mem_obj->get($video_new_video_list_data_key); //查看更多视频数据
if($data === false || (empty($mydata['gameid']) && $mydata['isflush'] > 0)){
    ...
    $data = $conn->find($sql);
    $mem_obj->set($video_new_video_list_data_key,$data,1800);
}
if(!empty($data)){
    foreach($data as $val){
