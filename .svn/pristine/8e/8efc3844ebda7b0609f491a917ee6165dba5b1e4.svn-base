...
$sql = "SELECT count(1) as num FROM `mzw_game` g,`mzw_game_version` gv WHERE g.g_id=gv.g_id  AND gv.g_id in(".$g_id_str.") AND gv.gv_id in(".$gv_id_str.") AND gv.gv_id in($tmp_sql_gpu_in)".$tmp_where;
...
$sql_data = "SELECT g.g_version_nums as historyVersionCount,gv.gv_id as appid, gv.gv_type_id as tid,gv.gv_title as title,gv.gv_version_no as versioncode,gv.gv_version_name as version,
gv.gv_publish_time as published, gv.gv_update_time as edittime, gv.gv_package_name as packagename,gv.gv_ico_key as icon,gv.gv_down_nums as downcount
FROM `mzw_game` g,`mzw_game_version` gv WHERE g.g_id=gv.g_id AND gv.g_id in(".$g_id_str.") AND gv.gv_id in(".$gv_id_str.") AND gv.gv_id in($tmp_sql_gpu_in) $tmp_where $orderby LIMIT ".$ParamPage.",".$mydata['pagesize'];
