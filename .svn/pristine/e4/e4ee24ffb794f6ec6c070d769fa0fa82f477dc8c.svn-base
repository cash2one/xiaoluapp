...
if($mydata['category']==-3){//如果是 模拟器游戏分类ID( 模拟器游戏的标签ID为：1 NES 2 PSP 3 MAME 根据需求不返回NES跟MAME游戏)
	$where .=  " AND  (FIND_IN_SET(2,gv.gv_nes_property)>0) ";
}else{
...
$data = array();
//查总数据行数
$sql = "SELECT count(1) as num FROM `mzw_game` g,`mzw_game_version` gv WHERE (FIND_IN_SET(1,gv.gv_client_type)>0 OR FIND_IN_SET(3,gv.gv_client_type)>0) AND  AND gv.gv_id IN($tmp_sql_gpu_in) $where ";
