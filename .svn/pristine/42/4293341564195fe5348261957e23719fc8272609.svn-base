...
if (count($game_list)>0){
    ...
    for ($i=0;$i< $game_count; $i++){
        ...
        if($mydata['language']=='en'){
            $game_version_where .= ' AND   gv_title_en!="" ';
        }
        $sql = "SELECT * FROM `mzw_game_version` WHERE (FIND_IN_SET(1,gv_client_type)>0 OR FIND_IN_SET(3,gv_client_type)>0) AND gv_package_name='$PackageName' AND ((gv_version_no>$VersionCode AND gv_status=1) OR gv_version_no=$VersionCode)  {$game_version_where} ORDER BY gv_version_no DESC";
        //echo($sql);
        //echo("<br>".chr(10));
        ...
        if(count($_row)>1){
			...
			*/
            ...
            switch ($mydata['language']) {
            	case 'zh':
            	    $json['title'] = $_row[$tmp_new_game['gv_id']]['gv_title'];//游戏名字
            	    break;
            	case 'en':
            	    $json['title'] = $_row[$tmp_new_game['gv_id']]['gv_title_en'];//英文游戏名字
            	    break;
            	default:
            	    $json['title'] = $_row[$tmp_new_game['gv_id']]['gv_title'];//游戏名字
            	    break;
