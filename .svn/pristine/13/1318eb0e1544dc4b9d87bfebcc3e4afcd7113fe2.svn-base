...
$sql_count = "SELECT count(*) as num "
		...
		WHERE (FIND_IN_SET(1,A.gv_client_type)>0 OR FIND_IN_SET(3,A.gv_client_type)>0) AND A.gv_status=1 AND B.ga_id =".$tmp_ga_id.$tmp_where;
...
$sql = "SELECT A.g_id as gid,A.gv_id as appid,A.gv_type_id as tid,A.gv_title as title,A.gv_title_en as title_en,A.gv_version_name as version,A.gv_update_time as updatetime,
		...
		. "FROM mzw_game_version A LEFT JOIN mzw_game_m_a_relation B ON A.g_id = B.g_id
		WHERE (FIND_IN_SET(1,A.gv_client_type)>0 OR FIND_IN_SET(3,A.gv_client_type)>0) AND A.gv_status=1 AND B.ga_id =".$tmp_ga_id.$tmp_where." ORDER BY $tmp_order_by $limit";
$data = $conn->find($sql);
...
foreach ($data as $val){
	...
	//查所属分类的名称
	$tmp_sql = 'SELECT t_name_cn as name, t_name_en as name_en FROM mzw_game_type WHERE t_id='.$val["tid"];
	...
	switch ($mydata['language']) {
		case 'zh':
		    $json['title'] = $val['title'];//游戏名字
		    break;
		case 'en':
		    $json['title'] = $val['title_en'];//英文游戏名字
		    break;
		default:
		    $json['title'] = $val['title'];//游戏名字
		    break;
