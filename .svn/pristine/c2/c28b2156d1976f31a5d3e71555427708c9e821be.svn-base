...
//查分类的信息
$sql = "SELECT t_id as tid,t_name_cn as title,t_name_en as title_en,t_img_key as icom,t_p_id AS type 
		FROM mzw_game_type WHERE t_status = 1 AND (t_type = 1 OR t_type=2) ORDER BY t_order_num DESC";
$data = $conn->find($sql);
...
switch ($mydata['language']) {
	case 'en':
	    $json_a['title'] = 'New';//最新游戏
	    $json_b['title'] = 'Large';//大型游戏
	    $json_c['title'] = 'Emulator';//模拟器
	    $json_d['title'] = 'Multiplayer';//多人游戏
	    break;
}
...
if($data){
	foreach($data as $row){
		...
		switch ($mydata['language']) {
			case 'zh':
			    $json['title'] =$row['title'];//分类名字
			break;
			case 'en':
			    $json['title'] =$row['title_en'];//英文分类名字
			break;
			default:
			    $json['title'] =$row['title'];//分类名字
			break;
