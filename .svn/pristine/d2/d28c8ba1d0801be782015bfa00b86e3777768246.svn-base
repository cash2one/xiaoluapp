...
$sql = "SELECT gv_id as appid,g_id as gid,gv_type_id as tid,gv_title as title,gv_version_no as versioncode,gv_version_name as version,
	...
	case gv_lang when 0 then '英文' when 1 then '中文' when 2 then '日文' when 3 then '韩文' when 4 then '其它' end language
		FROM `mzw_game_version`  WHERE (FIND_IN_SET(1,gv_client_type)>0 OR FIND_IN_SET(3,gv_client_type)>0) AND gv_status=1 AND gv_id=".$mydata['appid'];
...
if(($tmp_gpu_id_arr && count($tmp_gpu_id_arr)>0) && $data_arr){
    ...
    //查文件大小及游戏是APK还是GPK
    $tmp_sql = 'SELECT mgd_id,mgd_package_file_size as size,mgd_package_type as type,mgd_mzw_server_url,mgd_baidu_url,mgd_package_file_size,mgd_apk_agsin,mgd_game_unzip_size as unzip_size FROM mzw_game_downlist '
	...
	$sign = isset($data['sign']) ? $data['sign'] : '';//游戏签名
