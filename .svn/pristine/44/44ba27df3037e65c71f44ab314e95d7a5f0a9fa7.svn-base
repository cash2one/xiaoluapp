<?php
/*=============================================================================
#     FileName: tmp_action.sdk_game_show.in.php
#         Desc: 定期把记录在文件中的数据记录(/countapi/sdk_login_count.php)数据存在数据库存中(eid=100005)
#       Author: cai
#        Email: ddcai@163.com
...
//组合要入库文件的路径
$path = WEBPATH_DIR."data/sdk_game_show/data".date("Ymd",THIS_DATETIME - 86400)."_".$tmp_aa.".dat";

//随机取内容
$tmp_arr = file($path);
if(is_array($tmp_arr)){
	$tmp_table_name = "kyx_sdk_game_show_log";//数据表名字
    ...
    unset($tmp_arr);
    echo('导入sdk_game_show数据成功');
}else{
	echo('没有可导入的sdk_game_show数据');
}
