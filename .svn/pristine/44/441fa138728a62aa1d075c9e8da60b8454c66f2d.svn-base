...
class usermodel {
	 ...
	 * @description: 用户名或者手机号码登录
	 * @param: 
	 * @return: 
	 * @author: Xiong Jianbang
	 * @create: 2015-9-17 上午11:42:47
	 **/
...
// 	    if($this->check_phoneformat($username)){//手机号码
// 	        $arr = $this->db->fetch_first("SELECT * FROM ".UC_DBTABLEPRE."members WHERE phone='$username'");
// 	    }else{
// 	        $arr = $this->db->fetch_first("SELECT * FROM ".UC_DBTABLEPRE."members WHERE username='$username'");
// 	    }
// 		return $arr;
	 ...
	 * @param:$pwd=密码
	 * @author: Xiong Jianbang
	 * @create: 2015-10-8 下午3:51:19
	 **/
	function reset_pwd($username,$pwd){
	    if(empty($username) || empty($pwd)){
	        return FALSE;
	    }
	    $user = $this->get_user_by_username($username);
	    if(empty($user) || empty($user['salt'])){
	    	return FALSE;
	    }
	    //密码的加密算法
	    $passwordmd5 = preg_match('/^\w{32}$/', $pwd) ? $pwd : md5($pwd);
	    $newpwd = md5($passwordmd5.$user['salt']);
	    $time = time();
	    $this->db->query("UPDATE ".UC_DBTABLEPRE."members SET password='{$newpwd}' WHERE username='$username'");
	    $this->db->query("UPDATE ".UC_DBTABLEPRE."phone_reg SET is_end=1,updated={$time}  WHERE phone='$username' AND type=1");  //将状态改为完成
// 	    if($this->check_phoneformat($username)){//手机号码
// 	    }else{
// 	    }
	    return TRUE;
	...
	/**
	 * @name:check_phone
	 * @description: 检查手机号，其实是将手机号作为用户名了
	 * @param: 
	 * @return: 
	 * @author: Xiong Jianbang
	 * @create: 2015-11-3 下午5:29:51
	 **/
	...
	/**
	 * @name:check_emailformat
	 * @description: 检查email格式
	 * @author: Xiong Jianbang
	 * @create: 2015-9-16 下午6:25:40
	 **/
	...
	function add_user($username, $password, $email, $uid = 0, $questionid = '', $answer = '', $regip = '',$mac='',$source=2) {
		...
		//昵称
		$nickname = '';
	    if($this->check_phoneformat($username)){
	        $nickname= '用户'.substr($username,-6);//昵称默认为“用户＋手机后6位”
	    }
		$this->db->query("INSERT INTO ".UC_DBTABLEPRE."members SET $sqladd username='{$username}', password='{$password}', 
		        email='$email', regip='$regip', regdate='".$this->base->time."', salt='$salt',nickname='{$nickname}',gender=2,mac='{$mac}',source={$source}");
	...
	function edit_user_info($username, $nickname, $gender, $desc){
	    $gender = intval($gender);
	    $this->db->query("UPDATE ".UC_DBTABLEPRE."members SET nickname='{$nickname}',gender=$gender,`desc`='{$desc}'  WHERE username='$username'");
	    return $this->db->affected_rows();
	}
	
	
	function edit_user_login_info($username, $loginip){
	    $time = time();
	    $this->db->query("UPDATE ".UC_DBTABLEPRE."members SET lastloginip='{$loginip}',lastlogintime=$time  WHERE username='$username'");
	    return $this->db->affected_rows();
	}
	...
	function get_same_nickname($nickname='',$phonenumber=''){
		if(empty($nickname) || empty($phonenumber)){
			return  FALSE;
		}
		$sql = "SELECT uid FROM ".UC_DBTABLEPRE."members WHERE nickname='$nickname' AND username<>'{$phonenumber}'";
    ...
    function get_user_list($page, $pagesize, $sqladd) {
        $start = ($page - 1) * $pagesize;
        $data = $this->db->fetch_all("SELECT uid,username,email,nickname,gender,`desc` FROM ".UC_DBTABLEPRE."members $sqladd LIMIT $start, $pagesize");
        return $data;
    }

    function get_user_totalnum($sqladd) {
        $totalnum = $this->db->result_first("SELECT COUNT(*) FROM ".UC_DBTABLEPRE."members $sqladd");
        return $totalnum;
    }
