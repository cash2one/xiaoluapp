...
$mydata['type'] = intval(get_param('type'));// 0:游戏专区 1：软件专区
...
//查询条件
if(empty($mydata['type'])){
    $tmp_where = ' AND A.`gv_m_status` = 1 ';
}else{
    $tmp_where = ' ';
}
...
//查数据条数
$sql_count = "SELECT count(*) as num FROM mzw_game_version A LEFT JOIN mzw_game_m_a_relation B ON A.g_id = B.g_id AND (A.`gv_id` = B.`gv_id` OR B.`gv_id` =0)
              WHERE (FIND_IN_SET(2,A.gv_client_type)>0 OR FIND_IN_SET(3,A.gv_client_type)>0) AND B.ga_id = ".$tmp_ga_id.$tmp_where;
...
$sql = "SELECT A.g_id as gid,A.gv_id as appid,A.gv_type_id as tid,A.gv_title as title,A.gv_version_name as version,A.gv_update_time as updatetime,A.gv_description as description,
		...
		. "FROM mzw_game_version A LEFT JOIN mzw_game_m_a_relation B ON A.g_id = B.g_id AND (A.`gv_id` = B.`gv_id` OR B.`gv_id` =0)
		WHERE (FIND_IN_SET(2,A.gv_client_type)>0 OR FIND_IN_SET(3,A.gv_client_type)>0)
...
if(!empty($data)){
    foreach ($data as $val){
        ...
        //拼接查询下载地址条件
        $where_str = ' WHERE `mgd_client_type` != 1 AND `gv_id` = '.$val['appid'].' AND `mgd_package_type` != 2 ';
