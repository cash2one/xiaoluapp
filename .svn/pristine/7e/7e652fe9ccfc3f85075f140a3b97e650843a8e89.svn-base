...
if($mydata['category']==-1){//如果是最新游戏的分类ID，则一定是按最新来排序
	$where .=  " AND (gv.gv_nes_property = '' OR gv.gv_nes_property = 0 OR gv.gv_nes_property is null) ";
}
...
if($mydata['category']==-3){//如果是 模拟器游戏分类ID( 模拟器游戏的标签ID为：1 NES 2 PSP 3 MAME 根据需求不返回NES跟MAME游戏)
