...
$video_rank_list_count_key = 'video_rank_list_count_'.$mydata['gameid'].'_'.$mydata['order']; //排行榜视频总数缓存key  'video_rank_list_count_' + 游戏id + 排序类型id
$data_count = $mem_obj->get($video_rank_list_count_key); //排行榜视频总数
$data_count = false;
if($data_count === false){
    $sql = "SELECT COUNT(1) AS num FROM `video_video_list` " . $where . $time_where;
...
//获取排行榜数据
$video_rank_list_data_key = 'video_rank_list_data_' . $mydata['gameid'] . '_' . $mydata['order'] . '_' . $mydata['pagenum'] . '_' . $mydata['pagesize']; //排行榜视频数据缓存key  'video_rank_list_data_' + 游戏id + 排序类型id + 当前页 + 每页显示数据
...
if (!empty($data)) {
    ...
    $mem_obj->set($video_rank_list_data_key, $returnArr, 1800);
}
