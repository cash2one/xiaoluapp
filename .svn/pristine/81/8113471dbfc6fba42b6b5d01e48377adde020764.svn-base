...
//if( $data_arr){
	...
	//}
	$updatetime = isset($data['updatetime']) ? date('Y-m-d', strtotime($data['updatetime'])) : NULL;
...
//	}
	...
	$json = array(
			...
			'corver'=>isset($tmp_hot_arr[$data["appid"]][2])?$tmp_hot_arr[$data["appid"]][2][0]:'',//专题大图
			'icontvpath' =>isset($tmp_hot_arr[$data["appid"]][3])?$tmp_hot_arr[$data["appid"]][3][0]:'',//TV游戏大图
			'icontvindex' =>isset($tmp_hot_arr[$data["appid"]][5])?$tmp_hot_arr[$data["appid"]][5][0]:'',//首页大图
			'icontvdetail' =>isset($tmp_hot_arr[$data["appid"]][6])?$tmp_hot_arr[$data["appid"]][6][0]:'',//详情页大图
			'filetype'=>$filetype,//文件类型
            'fileType'=>$filetype,//文件类型
			'version'=>$data["version"],//游戏版本名称
			'versioncode'=>intval($data["versioncode"]),//游戏版本号
			'size'=>intval($size),//文件大小
            'unzipsize' => intval($tmp_downlist[0]['unzip_size']), //解压后文件大小
			'category'=>$category,//分类名称
			'updatetime'=>$data["updatetime"],//更新时间
			'downloadscount'=> intval($data["downloadscount"]),//下载次数
			'historyVersionCount'=> $online ? 0 : (isset($data["historyVersionCount"]) ? (intval($data["historyVersionCount"]) -1) : 0),//历史版本数量
			...
			'signature' => $sign,//游戏签名
			'propGameHandle'=>$propGameHandle,//游戏手柄
			'propMouseController'=>$propMouseController,////空鼠遥控器
			'propCommonController'=>$propCommonController,//普通遥控器
			'propCustomController'=>$propCustomController //支持自定义按键
	);
	
	//游戏相关
	if(!empty($data["tid"])){
	    $sql = "SELECT `gv_title` AS `title`,`gv_ico_key` as `icon`,`gv_id`,`gv_package_name` FROM `mzw_game_version` WHERE `gv_id`<> {$data['appid']} AND  `gv_type_id`={$data['tid']}  AND `gv_status`=1  ORDER BY RAND()  LIMIT 4 ";
	    $data = $conn->find($sql);
	    $arr_related = array();
	    if(!empty($data)){
...
//	            $tmp_game_ico = '';
//	            $tmp_sql = "SELECT A.id,size_id,A.extension,img_path,A.status,B.width,B.height FROM mzw_img_path A
//				LEFT JOIN mzw_img_size B ON A.size_id = B.id WHERE A.img_key = '".$value["icon"]."' AND B.width=100 AND B.height=100 AND A.status = 1 ORDER BY A.size_id";
//	            $tmp_game_ico_arr = $conn->get_one($tmp_sql);
//	            if($tmp_game_ico_arr){
//	                $tmp_game_ico = LOCAL_URL_DOWN_IMG.str_replace(LOCAL_IMG_PATH,"",$tmp_game_ico_arr["img_path"]);
//	            }
	            ...
	            $temp['appid'] = intval($value['gv_id']);
	            $temp['packagename'] = !empty($value['gv_package_name'])?trim($value['gv_package_name']):'';
	            $temp['iconpath'] = $tmp_game_ico;
	            $temp['title'] = trim($value['title']);
	            $temp['category'] =$category;
	            $arr_related[] = $temp;
	            unset($temp);
	        }
	    }
	    $json['related'] = $arr_related;
	}
	$returnArr['rows'][]=$json;
}
