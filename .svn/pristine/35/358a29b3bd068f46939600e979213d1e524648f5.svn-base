...
//查分类的信息
$sql = "SELECT t_id as tid,t_name_cn as title,t_img_key as icom,t_p_id AS type FROM mzw_game_type WHERE t_status = 1 AND (t_type = 1 OR t_type=2) ORDER BY t_order_num DESC";
$data = $conn->find($sql);
//查最新游戏数据(-1)
$json_a = array(
		'tid'=>-1,//分类ID
...
foreach ($data as $data_val){
	...
	$sql_data = "SELECT gv.gv_id as vid, gv.gv_type_id as tid,gv.gv_title as vtitle,gv.gv_version_no as versioncode,
			...
			" FROM `mzw_game_version` gv WHERE (FIND_IN_SET(1,gv.gv_client_type)>0 OR FIND_IN_SET(3,gv.gv_client_type)>0) AND gv.gv_id IN($tmp_sql_gpu_in)
			 $where  $orderby LIMIT 0,".$mydata['pagesize'];
	$game_data = $conn->find($sql_data);
	if($game_data){
		//查这个分类下总数据的个数
		$sql_data_count = "SELECT count(*) as num FROM `mzw_game_version` gv WHERE (FIND_IN_SET(1,gv.gv_client_type)>0 OR FIND_IN_SET(3,gv.gv_client_type)>0) AND gv.gv_id IN($tmp_sql_gpu_in)
			 $where ";
		...
		foreach ($game_data as $game_data_val){
			...
			$tmp_sql = 'SELECT mgd_id,mgd_package_file_size as size,mgd_package_type as type FROM mzw_game_downlist
			            WHERE mgd_client_type != 2 AND gv_id='.$game_data_val["vid"]." AND mgd_package_type!=2 ORDER BY mgd_package_type DESC,mgd_id DESC";
			...
			if($tmp_downlist){
				if(isset($tmp_downlist[0]) && $tmp_downlist[0]["type"]==1){//如果是ＧＰＫ
					...
					$tmp_sql_obb = 'SELECT id,mgd_id,apk_patch_size,patch_md5,sign,apk_patch_file,file_type FROM mzw_game_patch
				                    WHERE client_type != 2 AND gv_id='.intval($game_data_val["vid"]).' AND mgd_id='.intval($tmp_downlist[0]["mgd_id"]);
