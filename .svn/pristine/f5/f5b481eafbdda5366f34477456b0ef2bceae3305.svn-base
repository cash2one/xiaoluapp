...
/**
 * @copyright: @快游戏 2014
 * @description: 获取小鹿视频首页正在玩推荐数据列表,并JSON内容进行输出返回
 * @file: video_index_play_album.php
 * @author: Chen Zhong
 * @charset: UTF-8
...
//数据表名
$table_name = 'video_play_recom_video_list_'.$crc_num;
...
//获取视频数
$data_count_key = "xl_index_album_play_count_".md5($check_where);
...
//获取数据
$data_key = 'video_index_play_album_data_'.md5($check_where.$orderby.$limit);
...
if($data === false || $mydata['flushtype'] == 2){
    $sql = "SELECT B.`id`,B.`vvl_title`,B.`vvl_upload_time`,B.`vvl_imgurl`,B.`vvl_imgurl_get`,B.`vvl_category_id`,B.`vvl_up_num`,
            ...
            B.`vvl_uid`,B.`vvl_tags`,B.`vvl_count`,B.`vvl_time`,B.`vvl_video_id`,B.`vvl_game_id`,A.`status`
