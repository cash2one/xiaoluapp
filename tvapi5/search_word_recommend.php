...
if(!empty($data)){
	foreach ($data as $value) {
	    ...
	    $sql = "SELECT `gv_title` FROM  `mzw_game_version` WHERE `gv_id`={$gv_id} AND `gv_status`=1";
...
$sql = "SELECT `mhw_hotword`,`mhw_char` FROM `mzw_hot_word` WHERE `mhw_status`=1 AND `mhw_recommend`=1 AND (`mhw_type`=1 OR `mhw_type`=3) ORDER BY `mhw_weight` DESC";
$data = $conn->find($sql);
$arr_hot = array();
if(!empty($data)){
	foreach ($data as $value) {
...
$returnArr = array('rows'=>$arr,'total'=>count($arr),'pagecount'=>1,'pagenum'=>count($arr));
