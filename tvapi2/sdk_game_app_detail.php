...
$mydata['obbname'] = trim(get_param('obbname'));
$mydata['obbsize'] = rtrim(get_param('obbsize'),'_');
$mydata['arr_obbsize'] = array();
if(!empty($mydata['obbsize'])){
	$mydata['arr_obbsize'] = explode(',', $mydata['obbsize']);
	foreach ($mydata['arr_obbsize'] as $key=>$value) {
		$mydata['arr_obbsize'][$key] = rtrim($value,'_');
	}
}
...
if( is_empty($data_arr) ){
	...
	$sql = "SELECT gv_id as appid,g_id as gid,gv_type_id as tid,gv_title as title,gv_version_no as versioncode,gv_version_name as version,
		...
		gv_description as description,gv_app_prop as prop,gv_notice as attention,gv_update_text as updateContent,
		case gv_lang when 0 then '英文' when 1 then '中文' when 2 then '日文' when 3 then '韩文' when 4 then '其它' end language
			FROM `mzw_game_version`  WHERE (FIND_IN_SET(1,gv_client_type)>0 OR FIND_IN_SET(3,gv_client_type)>0) AND gv_package_name='".$mydata['packname']."'";
...
foreach ($data_arr as $data){
    ...
    /* 新版获取下载地址 start*/
    $where_str = ' WHERE mgd_client_type!=2 AND gv_id='.$data["appid"].' AND mgd_package_up_v_code='.$mydata['versioncode'].' AND mgd_package_type!=2 AND  '.$tmp_find_gpu_id;
    //如果是NES游戏
	...
	if($tmp_downlist && count($tmp_downlist)>0){
		...
		//2016-02-24需求：根据OBB的名称和大小进一步过滤
	   if(!empty($mydata['obbname']) && !empty($mydata['obbsize']) && count($mydata['arr_obbsize'])==1){
	   		 	$tmp_sql_obb = 'SELECT id,mgd_id,apk_patch_size,patch_md5,sign,apk_patch_file,file_type FROM mzw_game_patch
				        WHERE client_type != 2 AND gv_id='.intval($data["appid"]).' AND file_type=2  ';
	   }else{
	   		$tmp_sql_obb = 'SELECT id,mgd_id,apk_patch_size,patch_md5,sign,apk_patch_file,file_type FROM mzw_game_patch
				        WHERE client_type != 2 AND gv_id='.intval($data["appid"]).' AND file_type=2   AND  mgd_id='.intval($tmp_downlist[0]["mgd_id"]);
	   }
		...
		if(isset($tmp_downlist[0]) && $tmp_downlist[0]["type"]==1){//如果是ＧＰＫ
			...
			if($tmp_obb && count($tmp_obb)>0){//如果有OBB文件
				...
				foreach($tmp_obb as $sub){
					...
					//根据OBB的名称和大小进一步过滤
					if(!empty($mydata['obbname']) && !empty($mydata['obbsize']) && count($mydata['arr_obbsize'])==1){
						if($sub['apk_patch_size']==$mydata['obbsize']  &&  strpos($sub["apk_patch_file"], $mydata['obbname']) !== FALSE){
							$filepath[] = array(
...
//根据obbsize和obbname进一步过滤结果 2016-03-02  edit by xiongjianbang
$return = array();
if(!empty($returnArr['rows']) && !empty($mydata['obbsize']) && !empty($mydata['obbname']) && count($mydata['arr_obbsize'])==1){
	foreach ($returnArr['rows'] as $key=>$value) {
		$is_obb = false;
		 foreach ($value['downloadPaths'] as $v) { 
		 	if(!empty($v['files'])){
		 		foreach ($v['files'] as $v1) {
		 			if($v1['totalLength'] ==$mydata['obbsize'] &&  strpos($v1['fileName'], $mydata['obbname']) !== FALSE ) {
		 				$is_obb=true;;
		 			    break;
		 			}
		 		}
	...
	unset($returnArr['rows']);
	$returnArr['rows'] = $return['rows'];
	unset($return['rows']);
} 
