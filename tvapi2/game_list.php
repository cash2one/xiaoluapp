...
if($mydata['category']==-3){//如果是 模拟器游戏分类ID( 模拟器游戏的标签ID为：1)
	$where .=  " AND  FIND_IN_SET(1,gv.gv_nes_property)>0 "." AND gv.gv_nes_property!=2 ";
}
if($mydata['category']==-4){//如果是 支持自定义按键的游戏分类ID(支持自定义按键的 ID为：5)
	$where .=  " AND  FIND_IN_SET(5,gv.gv_app_prop)>0 "." AND gv.gv_nes_property!=2 ";
}
//===========begin适配GPU
//查找适配的GPU
...
foreach ($data as $key =>$v){
	...
	//查所属分类的名称
	$tmp_sql = 'SELECT t_name_cn as name FROM mzw_game_type WHERE t_id='.$v["tid"];
	...
	//查找这个游戏对应的相关图片
/*	$tmp_sql = 'SELECT img_key,path,type FROM mzw_game_screenshot WHERE gv_id='.$v["vid"]." ORDER BY id DESC ";
	$tmp_hot = $conn->find($tmp_sql);
	$tmp_hot_arr = array();//存放游戏对应的相关图片
	if($tmp_hot){
		foreach ($tmp_hot as $val_hot ){
			$tmp_hot_arr[$v["vid"]][$val_hot["type"]] = LOCAL_URL_DOWN_IMG.str_replace(LOCAL_IMG_PATH,"",$val_hot["path"]);
		}
	}
	*/
	//查找这个游戏对应的相关图片
