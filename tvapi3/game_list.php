...
if($mydata['category']==-1){//如果是最新游戏的分类ID，则一定是按最新来排序
	$where .=  " AND (gv.gv_nes_property not in(1,2,3) or gv.gv_nes_property is null) ";
}
if($mydata['sysversion']>100){
	if($mydata['category']>0){//如果是合法的分类ID，则
		$where .=  " AND  gv.gv_type_id=".$mydata['category']." AND gv.gv_nes_property!=2 ";
	}
	if($mydata['category']==-2){//如果是 大型游戏分类ID(大型游戏的标筌ID为：23)
		$where .=  " AND  FIND_IN_SET(23,gv.g_tags_property)>0 "." AND gv.gv_nes_property!=2 ";
	}
	if($mydata['category']==-5){//如果是 多人游戏分类ID( 多人游戏的标签ID为：24)
		$where .=  " AND  FIND_IN_SET(24,gv.g_tags_property)>0 "." AND gv.gv_nes_property!=2 ";
	}
}else{
	if($mydata['category']>0){//如果是合法的分类ID，则
		$where .=  " AND  gv.gv_type_id=".$mydata['category']." AND (gv.gv_nes_property='' OR gv.gv_nes_property is null) ";
	}
	if($mydata['category']==-2){//如果是 大型游戏分类ID(大型游戏的标筌ID为：23)
		$where .=  " AND  FIND_IN_SET(23,gv.g_tags_property)>0  AND (gv.gv_nes_property='' OR gv.gv_nes_property is null) ";
	}
	if($mydata['category']==-5){//如果是 多人游戏分类ID( 多人游戏的标签ID为：1)
		$where .=  " AND  FIND_IN_SET(24,gv.g_tags_property)>0  AND (gv.gv_nes_property='' OR gv.gv_nes_property is null) ";
	}
}
if($mydata['category']==-3){//如果是 模拟器游戏分类ID( 模拟器游戏的标签ID为：1)
	$where .=  " AND (FIND_IN_SET(1,gv.gv_nes_property)>0 OR FIND_IN_SET(2,gv.gv_nes_property)>0 OR FIND_IN_SET(3,gv.gv_nes_property)>0) ";
}
...
$data = array();
//查总数据行数
$sql = "SELECT count(1) as num FROM `mzw_game` g,`mzw_game_version` gv WHERE (FIND_IN_SET(1,gv.gv_client_type)>0 OR FIND_IN_SET(3,gv.gv_client_type)>0) AND gv.gv_id IN($tmp_sql_gpu_in) $where ";
	...
	if($tmp_downlist){
		if(isset($tmp_downlist[0]) && $tmp_downlist[0]["type"]==1){//如果是ＧＰＫ
			...
			$tmp_sql_obb = 'SELECT id,mgd_id,apk_patch_size,patch_md5,sign,apk_patch_file,file_type FROM mzw_game_patch
				            WHERE client_type != 2 AND gv_id='.intval($v["vid"]).' AND mgd_id='.intval($tmp_downlist[0]["mgd_id"]);
