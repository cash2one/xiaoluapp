...
if($mydata['size']!=0 && $mydata['size']!=1){
	...
	$where .= ' AND gv.gv_id IN (SELECT gv_id FROM mzw_game_downlist 
			   WHERE '.$tmp_find_gpu_id.' AND mgd_client_type != 2 GROUP BY gv_id HAVING max(mgd_game_size)'.$mydata['size'].') ';
}else{
...
//查数据
$sql_data = "SELECT gv.gv_id as vid, gv.gv_type_id as tid,gv.gv_title as vtitle,gv.gv_title_en as vtitle_en,gv.gv_version_no as versioncode,
...
//查所属分类的名称
$tmp_sql = 'SELECT t_id,t_name_cn as name,t_name_en as name_en FROM mzw_game_type WHERE t_status=1';
