...
<?php
/*=============================================================================
#     FileName: update_game_chl_model_count_redis.php
#         Desc: 每天统计 游戏 + 渠道 +盒子 + MAC 的注册 及登陆 数量
#       Author: chengdongcai
#        Email: ddcai@163.com
...
//游戏渠道.盒子型号统计(统计 1、总注册，2、总登录，3、总游戏时间)
$sql = "SELECT `grml_title`,`grml_pn`,`grml_chl`, `grml_md`,count(DISTINCT if(`grml_in_date`=$mydata,`grml_mac`,0))-1 as num,
count(DISTINCT `grml_mac`) as lnum,sum(grml_ut) as grml_ut_all
FROM `kyx_game_reg_mac_login` 
WHERE `grml_login_date` = $mydata 
group by `grml_pn`,`grml_chl`,`grml_md`";
...
if($row){
	...
	foreach ($row as $val){
		...
		$arr = array(
			...
			'gcm_reg_num'=>intval($val['num']),//'新增人数(当天注册人数)',
			'gcm_login_num'=>intval($val['lnum']),//'登陆人数(活跃数)',
			'gcm_ut'=>intval($val['grml_ut_all'])//'游戏平均时间(当天总游戏时间/活跃数)'
		);
		$conn->save('kyx_game_chl_model_count', $arr);
