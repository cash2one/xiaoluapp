...
<?php
/*=============================================================================
#     FileName: update_chl_model_count_redis.php
#         Desc: 每天更新统计 渠道 +盒子 + MAC 的注册 及登陆 数量
#       Author: chengdongcai
#        Email: ddcai@163.com
...
//渠道.盒子型号统计(统计 1、总注册，2、总登录，3、总游戏时间)
$sql = "SELECT `grml_chl`, `grml_md`,count(DISTINCT if(`grml_in_date`=$mydata,`grml_mac`,0))-1 as num,
count(DISTINCT `grml_mac`) as lnum,sum(grml_ut) as grml_ut_all
FROM `kyx_game_reg_mac_login` 
WHERE `grml_login_date` = $mydata 
group by `grml_chl`,`grml_md`";
...
if($row){
	...
	foreach ($row as $val){
		...
		$arr = array(
		  ...
		  'm_id'=>$model_id,//'盒子型号ID(来自kyx_model表)',
		  'cm_reg_num'=>intval($val['num']),//'新增人数(当天注册人数)',
		  'cm_login_num'=>intval($val['lnum']),//'登陆人数(活跃数)',
		  'cm_ut'=>intval($val['grml_ut_all'])//'游戏总时间(当天游戏总时间)'
		);
		$conn->save('kyx_chl_model_count', $arr);
