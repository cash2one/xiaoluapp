...
<?php
/*=============================================================================
#     FileName: update_game_device_count_redis.php
#         Desc: 每天更新统计 游戏 +手柄 + MAC 的注册  数量(即使用人数)
#       Author: chengdongcai
#        Email: ddcai@163.com
...
$sql = "SELECT `grml_title`,`grml_pn`,`grml_dc`,count(DISTINCT if(`grml_in_date`=$mydata,`grml_mac`,0)) as num,
count(DISTINCT `grml_mac`) as lnum
FROM `kyx_game_reg_mac_login` 
WHERE `grml_login_date` = $mydata 
group by `grml_pn`,`grml_dc`";
...
if($row){
	...
	foreach ($row as $val){
		...
		$conn->save('kyx_game_device_count', $arr);
