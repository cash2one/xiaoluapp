...
$mydata['source'] = 3;//访问来源（1：app 2:SDK 3:开放平台）
...
$sql_data = "SELECT `id`,`vvl_title`,`vvl_playurl`,`vvl_playurl_get`,`vvl_time`,`vvl_count`,`vvl_server_url`,
		     ...
		     `vvl_video_id`,`vvl_game_id`,`vvl_update_time` FROM `video_video_list`
		     WHERE `vvl_recommend` = 1 AND `id` = ".$mydata['appid'] ;
...
if(count($video_info)>0){
    ...
    $returnArr['rows'][0] = array(
        ...
        'updatetime' => intval($video_info['vvl_update_time']) //更新时间
    );
