...
/**
 * @copyright: @快游戏 2014
 * @description: 获取视频详情页相关视频
 * @file: video_relate_video_list.php
 * @author: Chen Zhong
 * @charset: UTF-8
 * @time: 2015-12-15  14:44
 * @version 1.0
 **/
...
//查询条件
$where = " WHERE A.`va_isshow`=1 AND CHAR_LENGTH(A.`vvl_title`)>=5 AND A.`id` != ".$mydata['appid'];

//获取视频关联游戏、关联专辑、关联作者
$sql_data = "SELECT `vvl_game_id`,`vvl_author_id`,`vvl_uid`,`vvl_category_id` FROM `video_video_list` WHERE `id` = ".$mydata['appid'];
$video_info = $conn->get_one($sql_data);

//游戏关联
if(isset($video_info['vvl_game_id']) && !empty($video_info['vvl_game_id'])){
    if($video_info['vvl_game_id'] == 2){
        $where .= " AND (A.`vvl_game_id` = 12 OR A.`vvl_game_id` = 2) ";
    }else{
        $where .= " AND A.`vvl_game_id` = ".$video_info['vvl_game_id'];
    }
}
...
}elseif(!empty($video_info['vvl_uid'])){
    $where .= " AND A.`vvl_uid` = ".$video_info['vvl_uid'];
}elseif(!empty($video_info['vvl_author_id'])){
    $where .= " AND A.`vvl_author_id` = ".$video_info['vvl_author_id'];
}

//查询数据总数
$video_relate_video_list_count_key = 'mzw_video_relate_video_list_count_'.$mydata['appid']; //视频详情页相关视频总数缓存key  'mzw_video_relate_video_list_count_' + 视频id
$data_count = $mem_obj->get($video_relate_video_list_count_key); //视频详情页相关视频总数
if($data_count === false){
    $sql = "SELECT count(1) as num FROM `video_video_list` AS A ".$where;
    $data_count = $conn->count($sql);
    $mem_obj->set($video_relate_video_list_count_key,$data_count,1800);
}
...
//获取视频更多数据
$video_relate_video_list_data_key = 'mzw_video_relate_video_list_data_'.$mydata['appid'].'_'.$mydata['pagenum'].'_'.$mydata['pagesize']; //视频详情页相关视频数据缓存key  'mzw_video_relate_video_list_data_' + 视频id + 当前页 + 每页显示数据
$data = $mem_obj->get($video_relate_video_list_data_key); //视频详情页相关视频数据
if($data === false){
    ...
    $data = $conn->find($sql);
    $mem_obj->set($video_relate_video_list_data_key,$data,1800);
}
if(!empty($data)){
    foreach($data as $val){
