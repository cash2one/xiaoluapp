<?php
/*=============================================================================
#     FileName: tmp_action.sdk_game_install_pos.count.in.php
#         Desc: 定期把记录在文件中的数据记录(/countapi/sdk_login_count.php)数据存在数据库存中（SDK内游戏点击安装位置）
#       Author: Chen Zhong
#        Email: chenzhong@kuaiyouxi.com
#   LastChange: 2015-06-01 11:18:48
#      History:
=============================================================================*/
...
//组合要入库文件的路径
$path = WEBPATH_DIR."data/sdk_game_install_pos/data".date("Ymd",THIS_DATETIME - 86400)."_".$tmp_aa.".dat";

//随机取内容
$tmp_arr = file($path);
if(is_array($tmp_arr)){
    $tmp_table_name = "kyx_sdk_game_install_pos_log";//数据表名字
    $str_sql = "insert into ".$tmp_table_name."(`sgipl_in_date`, `sgipl_md`, `sgipl_bd`, `sgipl_dc`, `sgipl_sdkv`,
			`sgipl_sdkbv`, `sgipl_vc`, `sgipl_vn`, `sgipl_pn`, `sgipl_title`, `sgipl_mac`, `sgipl_chl`, `sgipl_eid`, `sgipl_ct`,
			 `sgipl_ut`, `sgipl_st`, `sgipl_ip`, `sgipl_pos`,`sgipl_nte`)values";
    ...
    unset($tmp_arr);
    echo('导入sdk_game_install_pos数据成功');
}else{
    echo('没有可导入的sdk_game_install_pos数据');
}
