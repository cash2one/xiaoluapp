<?php
/*=============================================================================
#     FileName: tmp_action.sdk_game_insert.in.php
#         Desc: 定期把记录在文件中的数据记录(/countapi/sdk_login_count.php)数据存在数据库存中(eid=100007)
#       Author: cai
#        Email: ddcai@163.com
...
//组合要入库文件的路径
$path = WEBPATH_DIR."data/sdk_game_insert/data".date("Ymd",THIS_DATETIME - 86400)."_".$tmp_aa.".dat";

//随机取内容
$tmp_arr = file($path);
if(is_array($tmp_arr)){
	$tmp_table_name = "kyx_sdk_game_insert_log";//数据表名字
	$str_sql = "insert into ".$tmp_table_name."(`sgsl_in_date`, `sgsl_md`, `sgsl_bd`, `sgsl_dc`, `sgsl_sdkv`, 
			`sgsl_sdkbv`, `sgsl_vc`, `sgsl_vn`, `sgsl_pn`, `sgsl_title`, `sgsl_mac`, `sgsl_chl`, `sgsl_eid`, `sgsl_ct`,
			 `sgsl_ut`, `sgsl_st`, `sgsl_ip`, `sgsl_adappid`, `sgsl_advc`, `sgsl_advn`, `sgsl_adpn`, `sgsl_adtitle`,`sgsl_nte`)values";
    ...
    unset($tmp_arr);
    echo('导入sdk_game_insert数据成功');
}else{
	echo('没有可导入的sdk_game_insert数据');
}
