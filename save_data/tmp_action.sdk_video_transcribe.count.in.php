<?php
/*=============================================================================
#     FileName: tmp_action.sdk_video_transcribe.count.in.php
#         Desc: 定期把记录在文件中的数据记录(/countapi/sdk_login_count.php)数据存在数据库存中（SDK视频录制日志入库）
#       Author: Chen Zhong
#        Email: chenzhong@kuaiyouxi.com
#   LastChange: 2015-07-20 15:06:48
#      History:
=============================================================================*/
...
//组合要入库文件的路径
$path = WEBPATH_DIR."data/sdk_video_transcribe/data".date("Ymd",THIS_DATETIME - 86400)."_".$tmp_aa.".dat";

//随机取内容
$tmp_arr = file($path);
if(is_array($tmp_arr)){
    $tmp_table_name = "kyx_sdk_video_transcribe_log";//数据表名字
    $str_sql = "insert into ".$tmp_table_name."(`svtl_in_date`, `svtl_md`, `svtl_bd`, `svtl_dc`, `svtl_sdkv`,
			    `svtl_sdkbv`, `svtl_vc`, `svtl_vn`, `svtl_pn`, `svtl_title`, `svtl_mac`, `svtl_chl`, `svtl_eid`, `svtl_ct`,
			    `svtl_ut`, `svtl_st`, `svtl_ip`, `svtl_imei`,`svtl_lt`,`svtl_vt`,`svtl_nt`,`svtl_nte`)values";
    ...
    foreach($tmp_arr as $v){
        if(!empty($v)){
            ...
            $tmp_sql_val .= ",'".$val['imei']."'";//imei串号
            $tmp_sql_val .= ",".intval($val['lengthoftime']);//录制时长
            $tmp_sql_val .= ",".(isset($val['vt']) ? ("'".$val['vt']."'") : "''")."";//视频标题
            $tmp_sql_val .= ",".(isset($val['nte']) ? ("'".$val['nte']."'") : "''")."";//网络环境
    ...
    unset($tmp_arr);
    echo('导入sdk_video_transcribe数据成功');
}else{
    echo('没有可导入的sdk_video_transcribe数据');
}
