<?php
/*=============================================================================
#     FileName: tmp_action_export.sdk_game_show.count.in.php
#         Desc: 每天凌晨将两天前的数据导入到对应月的表中 kyx_sdk_game_show_log_$month
#       Author: Chen Zhong
#        Email: chenzhong@kuaiyouxi.com
#   LastChange: 2015-06-02 17:09:35
#      History:
=============================================================================*/
//TODO
//这个程序需要注意可能会出现多天的数据
...
$sql   = "SELECT LEFT(sgsl_in_date,6) AS month FROM kyx_sdk_game_show_log GROUP BY LEFT(sgsl_in_date,6)";
$data = $conn->find($sql);
if($data && isset($data[0])){
	$row['month'] = $data[0]['month'];
    $str_check_sql = "CREATE TABLE IF NOT EXISTS `kyx_sdk_game_show_log_".$row['month']."` (
        ...
        ) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='SDK内游戏展示次数日志表';
     ";
    //检查当用的表是否存在，不存在则建表
    $conn->query($str_check_sql);

     //不归档当天的数据
	$sql = "INSERT INTO `kyx_sdk_game_show_log_".$row['month']."` SELECT * FROM kyx_sdk_game_show_log WHERE sgsl_in_date<".$tmp_this_day." and LEFT(sgsl_in_date,6)='".$row['month']."'";
    $rs = $conn->query($sql);

    // 从临时表中删除对应月的数据
    if($rs){
		//不归档当天的数据
		$sql = "DELETE FROM kyx_sdk_game_show_log WHERE sgsl_in_date<".$tmp_this_day." AND LEFT(sgsl_in_date,6)='".$row['month']."'";
		$conn->Query($sql);
		echo("SDK游戏展示数据当月归档成功");
	}else{
		echo("SDK游戏展示数据当月归档失败");
	}
}else{
    echo("查询SDK游戏展示数据出错");
}
